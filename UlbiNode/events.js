...
/*
    В Node.js текст и другие данные могут приходить через 
    переменные окружения (environment variables), потому 
    что это стандартный способ обмена информацией между 
    операционной системой, приложением и его окружением. 
    Переменные окружения позволяют передавать конфигурационные 
    данные, параметры или секреты (например, ключи API или параметры 
    базы данных), которые не должны быть жестко закодированы в 
    исходном коде приложения.

    Причины использования переменных окружения:
    Конфигурация вне кода: Использование переменных окружения 
    позволяет хранить конфигурацию и параметры приложения вне 
    его исходного кода. Это упрощает настройку и изменение 
    поведения приложения, не требуя изменений в самом коде.
    ...
    Флаги или настройки для разных этапов разработки, тестирования 
    и продакшн-окружения.
    Безопасность: Важные данные, такие как ключи доступа или пароли, 
    могут быть переданы через переменные окружения, чтобы избежать 
    их включения в исходный код, что помогает избежать утечек 
    информации. Например, с использованием .env файлов или облачных 
    сервисов, таких как Heroku, AWS, или Docker, переменные окружения 
    можно передавать в приложение без риска попадания в публичные 
    репозитории.

    Удобство настройки: Переменные окружения позволяют легко 
    менять конфигурацию приложения в разных средах (например, 
    на локальной машине, в тестовой среде или в продакшн). Вы можете 
    настроить различные значения переменных окружения для разных 
    сред, чтобы приложение вело себя по-разному в зависимости от 
    окружения.
*/
